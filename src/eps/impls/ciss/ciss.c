...
-  -eps_ciss_realmats (true|false) - $A$ and $B$ are real
...
-  quad - the quadrature rule, see `EPSCISSQuadRule` for possible values
   ...
   Options Database Key:
.  -eps_ciss_quadrule (trapezoidal|chebyshev) - sets the quadrature rule
...
-  extraction - the extraction technique, see `EPSCISSExtraction` for possible values
   ...
   Options Database Key:
.  -eps_ciss_extraction (ritz|hankel) - sets the extraction technique
