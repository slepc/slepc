...
.  -svd_trlanczos_oneside (true|false) - enable the one-sided variant
...
-  bidiag - the bidiagonalization choice, see `SVDTRLanczosGBidiag` for possible values
   ...
   Options Database Key:
.  -svd_trlanczos_gbidiag (single|upper|lower) - sets the bidiagonalization choice
...
-  lock - `PETSC_TRUE` if the locking variant must be selected
   ...
   Options Database Key:
.  -svd_trlanczos_locking (true|false) - sets the locking flag
...
-  explicitmat - Boolean flag indicating if $Z$ is built explicitly
   ...
   Options Database Key:
.  -svd_trlanczos_explicitmatrix (true|false) - enable the explicit construction of the stacked matrix
