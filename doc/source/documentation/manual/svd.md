...
:alt: Scheme of the thin SVD of a rectangular matrix.
:name: fig:svd

Scheme of the thin SVD of a rectangular matrix
```
...
and so if $s_i\neq 0$ then $A^*Ax_i=\sigma_i^2B^*Bx_i$, a generalized eigenvalue problem for the matrix pencil $(A^*A,B^*B)$. This is the analog of the cross product matrix eigenproblem of equation {math:numref}`eq:eigleft`.
...
where `A` can be any matrix, not necessarily square, stored in any allowed PETSc format including the matrix-free mechanism (see section [](#sec:supported) for a detailed discussion), and `B` is generally set to `NULL` except in the case of computing the GSVD. If the problem is hyperbolic (cf. [](#sec:hsvd)) then in addition a signature matrix $\Omega$ must be provided with:
...
:::{note}
The signature matrix $\Omega$ is represented as a vector containing values equal to $1$ or $-1$.
:::
...
For the selection of the portion of the spectrum of interest, there are only two possibilities in the case of SVD: largest and smallest singular values, see table [](#tab:whichsvd). The default is to compute the largest ones, but this can be changed with:
...
Illustration of threshold usage with the singular values of the `rdb200` matrix; the red line represents a relative threshold of {math}`\delta=0.8`
```
...
The available methods for computing the partial SVD are shown in table [List of solvers available in the SVD module](#tab:svdsolvers). These methods can be classified in the following categories:
...
while the user writes the options database command `-svd_type` followed by the name of the method (see table [List of solvers available in the SVD module](#tab:svdsolvers)).

:::{note}
Not all solvers in the `SVD` module support non-standard problems (GSVD and HSVD). [List of solvers available in the SVD module](#tab:svdsolvers) indicates which solvers can be used for these.
:::
...
Similarly, in the case of GSVD the thick-restart Lanczos solver uses a {external:doc}`KSP` solver internally, that can be configured by accessing it with:
...
where $\tilde\omega$ is the corresponding element of the signature $\tilde\Omega$ of the definition {math:numref}`eq:hsvd`.
