...
The example program is very similar to that in Exercise 1. The main difference is that the problem is set to be non-symmetric with [EPSSetProblemType](../../manualpages/EPS/EPSSetProblemType):

```{code} c
PetscCall(EPSSetProblemType(eps,EPS_NHEP));
```

In this example we also illustrate the use of [EPSSetInitialSpace](../../manualpages/EPS/EPSSetInitialSpace).
...
Other criteria can be used, see [EPSSetWhichEigenpairs](../../manualpages/EPS/EPSSetWhichEigenpairs) for details. For instance, for computing only the rightmost eigenvalues, try the following.
...
Sometimes, an improvement may come from changing the way in which the method extracts the spectral information from the built subspace; see [EPSSetExtraction](../../manualpages/EPS/EPSSetExtraction) for details. One such technique is called harmonic extraction. Try the following:
...
Each EPS object uses an ST object internally to manage the spectral transformation. The following table shows the available spectral transformations, which can be selected with the function [STSetType](../../manualpages/ST/STSetType) or at run time.
