...
def printindex(outfilename, headfilename, levels, titles, tables):
      ...
      if posixpath.exists(headfilename) :
            with open(headfilename, "r") as fd:
                headbuf = fd.read()
                headbuf = headbuf.replace('SLEPC_DIR', '../../../')
      else:
...
def printsingleindex(outfilename, alphabet_dict):
      global numberErrors
      with open(outfilename, "w") as fd:
          fd.write("# Single Index of all SLEPc Manual Pages\n\n")
      ...
      if m:
        ...
        if loc_html:
          ...
          if loc:
              ...
              buf += "\n\n---\n[Edit on GitLab](https://gitlab.com/slepc/slepc/-/edit/%s/%s)\n\n" % (edit_branch, source_path)
          else:
...
def main(SLEPC_DIR, build_dir):
      global numberErrors
      HEADERDIR = 'doc/source/manualpages/MANSECHeaders'
      ...
      for dirname in mandirs:
            ...
            headfilename = SLEPC_DIR + '/' + HEADERDIR + '/' + secname
