...
def processfile(slepc_dir,dir,file,keyre,mdict,uses):
...
def processdir(slepc_dir,dir,keyre,mdict,uses):
  '''Loop over tutorials, call processfile() on each'''
  #print('Processing '+dir)
  for file in os.listdir(dir):
    if os.path.isfile(os.path.join(dir,file)) and (file.endswith('.c') or file.endswith('.cxx') or file.endswith('.F90')): processfile(slepc_dir,dir,file,keyre,mdict,uses)

def loadmanualpagescit(slepc_dir,srcdir):
  ...
  with open(os.path.join(srcdir,'manualpages','manualpages.cit'),'r') as fd:
    text = fd.read()
...
def main(slepc_dir,srcdir):
    keyre,mdict,uses = loadmanualpagescit(slepc_dir,srcdir)
    for dirpath, dirnames, filenames in os.walk(os.path.join(slepc_dir,'src'),topdown=True):
      dirnames[:] = [d for d in dirnames if d not in ['output', 'ftn-custom', 'ftn-auto', 'ftn-mod', 'tests', 'binding']]
      if dirpath.endswith('tutorials'):
        processdir(slepc_dir,dirpath,keyre,mdict,uses)

    for i in mdict:
      if len(uses[i[1:-1]]) > 0:
        manpage = os.path.join(srcdir,'manualpages',mdict[i])
        set_uses = set(uses[i[1:-1]])
        uses[i[1:-1]] = []
        with open(manpage,'a') as fd:
          fd.write('\n## Examples\n')
          for j in set_uses:
            file = j.replace(slepc_dir+'/','')
