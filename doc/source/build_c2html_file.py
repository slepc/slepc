...
def _run_mapnames(slepc_dir,mapnames,txt):
  return subprocess.check_output(
    [mapnames, '-map', os.path.join(slepc_dir, 'htmlmap.tmp'), '-inhtml'],
    text=True,
    input=txt,
  )

def _write_html(slepc_dir,loc,git_sha,rel_dir,file,txt):
...
def _map_batch(slepc_dir,mapnames,texts):
  ...
  if batched:
    ...
    output = _run_mapnames(slepc_dir,mapnames,separator.join(texts[i] for i in batched)).split(separator)
...
def main(slepc_dir,loc,git_sha,c2html,mapnames,rel_dir,file):
  ...
  txt = _run_mapnames(slepc_dir,mapnames,txt)
  _write_html(slepc_dir,loc,git_sha,rel_dir,file,txt)

def main_batch(slepc_dir,loc,git_sha,c2html,mapnames,sourcefiles):
  ...
  for (rel_dir,file),txt in zip(files,_map_batch(slepc_dir,mapnames,texts)):
    _write_html(slepc_dir,loc,git_sha,rel_dir,file,txt)
  return len(files)
